...
function writeAmf3Object(buffer, value, info) {
  ...
  // Write object header
  // Format: (trait-ref:1=0, externalizable:1, dynamic:1, property-count:29) = 32 bits
  var header = 0;
  
  if (isExternalizable) {
    header = (1 << 2) | 1; // externalizable bit + inline trait marker
  } else {
    header = (1 << 3) | 1; // dynamic bit + inline trait marker
    header |= propertyNames.length << 4; // property count
  }
